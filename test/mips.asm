...
main:
	...
	subu	 $t2,$t1,$t3			#	 $t2 = ffff fffb	2c
	sub	 $t2,$t1,$t3			#	 $t2 = ffff fffb	30
...
l1:	
	...
	addi	$t1,$t2,1000			#shouldnt happen		88
l2:
	addi	$t1,$0,100			#	$t1=64 		8c
	sb	$t1,8($t2)			#	[0x1001 0008]= 0x64	90
	lb	$t3,8($t2)			#	$t3= [0x1001 0008]= 0x64	94
	bne	$t3,$0,l3			#	jump to 		98
	addi	$t1,$t2,1000			#shouldnt happen		a0
l3:	
	addi	$t4,$0,-100			#	$t4=0xffff ff9c		a4
	sh	$t4,12($t2)			#   [0x1001 000c]= 0x0000 ff9c	a8
	sw	$t4,4($t2)			#  [0x1001 0004]= 0xffff ff9c	ac
	lh	$t3,4($t2)			#  $t3=	0xffff ff9c		b0
	lhu	$t5,4($t2)			#  $t5= 0x0000 ff9c		b4
	lbu	$t4,4($t2)			#  $t5= 0x0000 ff9c		b4
